...
Map<String, dynamic> _writeToJsonMap(_FieldSet fs) {
  ...
  for (var fi in fs._infosSortedByTag) {
    ...
    if (_isMapField(fi.type)) {
      result['${fi.tagNumber}'] = _writeMap(value, fi as MapFieldInfo<dynamic, dynamic>);
      continue;
    }
...
void _mergeFromJsonMap(
  ...
  for (String key in keys) {
    ...
    if (fi.isMapField) {
      _appendJsonMap(fs, json[key], fi as MapFieldInfo<dynamic, dynamic>, registry);
...
void _appendJsonMap(
  ...
  PbMap map = fi._ensureMapField(fs) as PbMap<dynamic, dynamic>;
...
_convertJsonValue(_FieldSet fs, value, int tagNumber, int fieldType,
  ...
  switch (PbFieldType._baseType(fieldType)) {
    ...
    case PbFieldType._MESSAGE_BIT:
      if (value is Map) {
        Map<String, dynamic> messageValue = value as Map<String, dynamic>;
