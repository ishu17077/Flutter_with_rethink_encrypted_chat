...
typedef void FrozenMessageErrorHandler(String messageName,
    [String? methodName]);
...
class _FieldSet {
  ...
  List<T?> _$getList<T>(int index) {
    var value = _values[index];
    if (value != null) return value as List<T>;
    return _getDefaultList<T?>(
        _nonExtensionInfoByIndex(index) as FieldInfo<T?>);
  }
  ...
  void _mergeField(FieldInfo otherFi, fieldValue, {bool? isExtension}) {
    ...
    if (fi!.isMapField) {
      MapFieldInfo f = fi as MapFieldInfo<dynamic, dynamic>;
      mustClone = _isGroupOrMessage(f.valueFieldType);
      PbMap map = f._ensureMapField(this) as PbMap<dynamic, dynamic>;
      if (mustClone) {
